    ...
    while (!sortirBoucle) {
        ...
        switch (selection) {
            ...
            case '2' : {
                ...
                cin.ignore();
                getline(cin, fileName);
                message = decodeMessage(fileName);
 ...
 * @return La taille du fichier BMP.
 */
int getImageSizeFile(string fileName) {
 ...
 */
int
writeBMP(unsigned char binaryImage[], bitset<SIZE_OCTET> binaryMessage[], int fileSize, int binaryMessageArraySize) {
